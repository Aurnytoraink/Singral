...
import tidalgtk.api.playlist
import tidalgtk.api.request
import tidalgtk.api.user
import tidalgtk.api.media
import tidalgtk.api.artist
import tidalgtk.api.album
import tidalgtk.api.genre
...
SearchTypes = [tidalgtk.api.artist.Artist,
               tidalgtk.api.album.Album,
               tidalgtk.api.media.Track,
               tidalgtk.api.media.Video,
               tidalgtk.api.playlist.Playlist]
...
class Config(object):
    ...
    """
    def __init__(self, quality=Quality.lossless, video_quality=VideoQuality.high, item_limit=1000, alac=True):
        ...
        self.api_token = "_DSTon1kC8pABnTw"
...
class Session(object):
    ...
    def __init__(self, config=Config()):
        ...
        self.request = tidalgtk.api.request.Requests(session=self)
        self.genre = tidalgtk.api.genre.Genre(session=self)
        ...
        self.parse_user = tidalgtk.api.user.User(self, None).parse
    ...
    def load_session(self, session_id, country_code=None, user_id=None):
        ...
        self.user = tidalgtk.api.user.User(self, user_id=user_id)
        return True
    ...
    def change_quality(self, quality, video_quality):
        self.config = Config(quality=quality, video_quality=video_quality, alac=self.config.alac)
    ...
    def login(self, username, password):
        ...
        self.user = tidalgtk.api.user.User(self, user_id=body['userId']).factory()
        return True
    ...
    def playlist(self, playlist_id=None):
        ...
        Calls :class:`tidalgtk.api.Playlist(session=session, playlist_id=playlist_id) <.Playlist>` internally
        ...
        return tidalgtk.api.playlist.Playlist(session=self, playlist_id=playlist_id).factory()

    def track(self, track_id=None):
        """
        Function to create a Track object with access to the session instance in a smoother way.
        Calls :class:`tidalgtk.api.Track(session=session, track_id=track_id) <.Track>` internally
        ...
        return tidalgtk.api.media.Track(session=self, media_id=track_id)

    def video(self, video_id=None):
        """
        Function to create a Video object with access to the session instance in a smoother way.
        Calls :class:`tidalgtk.api.Video(session=session, video_id=video_id) <.Video>` internally
        ...
        return tidalgtk.api.media.Video(session=self, media_id=video_id)

    def artist(self, artist_id=None):
        """
        Function to create a Artist object with access to the session instance in a smoother way.
        Calls :class:`tidalgtk.api.Artist(session=session, artist_id=artist_id) <.Artist>` internally
        ...
        return tidalgtk.api.artist.Artist(session=self, artist_id=artist_id)

    def album(self, album_id=None):
        """
        Function to create a Album object with access to the session instance in a smoother way.
        Calls :class:`tidalgtk.api.Album(session=session, album_id=album_id) <.Album>` internally
        ...
        return tidalgtk.api.album.Album(session=self, album_id=album_id)

    def get_user(self, user_id=None):
        """
        Function to create a User object with access to the session instance in a smoother way.
        Calls :class:`tidalgtk.api.user.User(session=session, user_id=user_id) <.User>` internally
        ...
        return tidalgtk.api.user.User.User(session=self, user_id=user_id).factory()
